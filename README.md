...
- `README.md`: Short report explaining the approach and challenges.

## Approach
- **Data Structure**: The application uses a list filled with dictionaries (`tasks = []`). Each task dictionary stores the task's ID, description, and status.
...
## Challenges Faced
- **Incrementing IDs**: When tasks are deleted, the next task ID must still increment correctly. We resolved this by getting the ID of the last element in the list and adding 1 (`tasks[-1]["id"] + 1 if tasks else 1`).
